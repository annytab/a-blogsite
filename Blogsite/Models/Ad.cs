...
public class Ad
{
    ...
    /// <summary>
    /// Create a new ad with default properties
    /// </summary>
    public Ad()
    {
    ...
    /// <summary>
    /// Create a new ad from a reader
    /// </summary>
    /// <param name="reader">A reference to a reader</param>
    public Ad(SqlDataReader reader)
    {
        // Set values for instance variables
        this.id = Convert.ToInt32(reader["id"]);
        this.language_id = Convert.ToInt32(reader["language_id"]);
        this.name = reader["name"].ToString();
        this.ad_slot = reader["ad_slot"].ToString();
        this.ad_code = reader["ad_code"].ToString();
    ...
    /// <summary>
    /// Count the number of ads by search keywords
    /// </summary>
    /// <param name="keywords">An array of keywords</param>
    /// <returns>The number of ads as an int</returns>
    public static Int32 GetCountBySearch(string[] keywords)
    {
        ...
        string connection = Tools.GetConnectionString();
        string sql = "SELECT COUNT(id) AS count FROM dbo.ads WHERE 1 = 1";

        // Append keywords to the sql string
        for (int i = 0; i < keywords.Length; i++)
        {
            sql += " AND (name LIKE @keyword_" + i.ToString() + " OR ad_slot LIKE @keyword_" + i.ToString() + ")";
        }
    ...
    public static bool MasterPostExists(Int32 id)
    {
        ...
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT COUNT(id) AS COUNT FROM dbo.ads WHERE id = @id;";
    ...
    public static List<Ad> GetAll(string sortField, string sortOrder)
    {
        ...
        // Create the connection string and the select statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT * FROM dbo.ads ORDER BY " + sortField + " " + sortOrder + ";";
    ...
    /// <summary>
    /// Get ads based on language id
    /// </summary>
    /// <param name="languageId">The language id</param>
    /// <param name="sortField">The field to sort on</param>
    /// <param name="sortOrder">The sort order</param>
    /// <returns>A list of ad posts</returns>
    public static List<Ad> GetByLanguageId(Int32 languageId, string sortField, string sortOrder)
    {
        // Make sure that sort variables are valid
        sortField = GetValidSortField(sortField);
        sortOrder = GetValidSortOrder(sortOrder);
        ...
        // Create the list to return
        List<Ad> posts = new List<Ad>(10);

        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT * FROM dbo.ads WHERE language_id = @language_id ORDER BY " + sortField + " " + sortOrder + ";";
    ...
    /// <summary>
    /// Get ads based on a ad slot and language id
    /// </summary>
    /// <param name="adSlot">The ad slot name</param>
    /// <param name="languageId">The language id</param>
    /// <param name="sortField">The field to sort on</param>
    /// <param name="sortOrder">The sort order</param>
    /// <returns>A list of ads posts</returns>
    public static List<Ad> GetByAdSlot(string adSlot, Int32 languageId, string sortField, string sortOrder)
    {
        // Make sure that sort variables are valid
        sortField = GetValidSortField(sortField);
        sortOrder = GetValidSortOrder(sortOrder);
        ...
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT * FROM dbo.ads WHERE language_id = @language_id AND ad_slot = "
            + "@ad_slot AND inactive = @inactive ORDER BY " + sortField + " " + sortOrder + ";";
        ...
        using (SqlConnection cn = new SqlConnection(connection))
        {
            ...
            using (SqlCommand cmd = new SqlCommand(sql, cn))
            {
                // Add parameters
                cmd.Parameters.AddWithValue("@language_id", languageId);
                cmd.Parameters.AddWithValue("@ad_slot", adSlot);
    ...
    } // End of the GetByAdSlot method

    /// <summary>
    /// Get ads by search keywords
    /// </summary>
    /// <param name="keywords">An array of keywords</param>
    ...
    /// <param name="sortOrder">The sort order</param>
    /// <returns>A list of ads</returns>
    public static List<Ad> GetBySearch(string[] keywords, Int32 pageSize, Int32 pageNumber, string sortField, string sortOrder)
    {
        // Make sure that sort variables are valid
        sortField = GetValidSortField(sortField);
        sortOrder = GetValidSortOrder(sortOrder);
        ...
        // Create the list to return
        List<Ad> posts = new List<Ad>(pageSize);

        // Create the connection string and the select statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT * FROM dbo.ads WHERE 1 = 1";
    ...
    /// <summary>
    /// Delete a ad post on id
    /// </summary>
    /// <param name="id">The id of the ad post</param>
    /// <returns>An error code</returns>
    public static Int32 DeleteOnId(Int32 id)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "DELETE FROM dbo.ads WHERE id = @id;";
