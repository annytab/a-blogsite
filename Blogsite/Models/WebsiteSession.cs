...
/// <summary>
/// This class represent a website session
/// </summary>
public class WebsiteSession
{
    ...
    /// <summary>
    /// Create a new session with default properties
    /// </summary>
    ...
    /// <summary>
    /// Create a new session from a reader
    /// </summary>
    /// <param name="reader">A reference to a reader</param>
    public WebsiteSession(SqlDataReader reader)
    {
        // Set values for instance variables
    ...
    /// <summary>
    /// Update a session post
    /// </summary>
    /// <param name="post">A reference to a session post</param>
    public static void Update(WebsiteSession post)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET created_date = @created_date, expires_date = @expires_date, " 
    ...
    /// <summary>
    /// Update a session post with lock id
    /// </summary>
    /// <param name="post">A reference to a session post</param>
    public static void UpdateWithLockId(WebsiteSession post)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET expires_date = @expires_date, "
    ...
    /// <summary>
    /// Update the expiration date for a session
    /// </summary>
    /// <param name="post">A reference to a session post</param>
    public static void UpdateExpirationDate(WebsiteSession post)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET expires_date = @expires_date "
    ...
    /// </summary>
    /// <param name="post">A reference to a session post</param>
    public static Int32 Lock(WebsiteSession post)
    {
        ...
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET lock_date = @lock_date, locked = @locked "
    ...
    /// </summary>
    /// <param name="post">A reference to a session post</param>
    public static void Unlock(WebsiteSession post)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET expires_date = @expires_date, locked = @locked "
            + "WHERE id = @id AND application_name = @application_name AND lock_id = @lock_id;";
    ...
    public static void UpdateLockIdAndFlags(string id, string applicationName, Int32 lockId)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.website_sessions SET lock_id = @lock_id, flags = 0 "
            + "WHERE id = @id AND application_name = @application_name;";
    ...
    /// <summary>
    /// Get one session based on id
    /// </summary>
    /// <param name="id">The id</param>
    /// <param name="applicationName">The application name</param>
    /// <returns>A reference to a session post</returns>
    public static WebsiteSession GetOneById(string id, string applicationName)
    {
        // Create the post to return
        WebsiteSession post = null;

        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "SELECT * FROM dbo.website_sessions WHERE id = @id AND application_name = @application_name;";
        ...
        using (SqlConnection cn = new SqlConnection(connection))
        {
            ...
            using (SqlCommand cmd = new SqlCommand(sql, cn))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        post = new WebsiteSession(reader);
                    }
                }
                catch (Exception e)
                {
    ...
    public static void DeleteOnId(string id, string applicationName)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "DELETE FROM dbo.website_sessions WHERE id = @id AND application_name = @application_name;";
    ...
    /// <summary>
    /// Delete all sessions that has expired
    /// </summary>
    public static void DeleteAllExpired()
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "DELETE FROM dbo.website_sessions WHERE expires_date < @expires_date;";
