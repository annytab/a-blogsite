...
public class Category
{
    ...
    public static long AddMasterPost(Category post)
    {
        ...
        string sql = "INSERT INTO dbo.categories (parent_category_id, meta_robots) "
            + "VALUES (@parent_category_id, @meta_robots);SELECT SCOPE_IDENTITY();";
    ...
    public static void AddLanguagePost(Category post, Int32 languageId)
    {
        ...
        string sql = "INSERT INTO dbo.categories_detail (category_id, language_id, title, main_content, meta_description, " 
            + "meta_keywords, page_name, date_added, page_views, inactive) "
            + "VALUES (@category_id, @language_id, @title, @main_content, @meta_description, @meta_keywords, "
            + "@page_name, @date_added, @page_views, @inactive);";
    ...
    public static void UpdateMasterPost(Category post)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.categories SET parent_category_id = @parent_category_id, meta_robots = @meta_robots WHERE id = @id;";
    ...
    public static void UpdateLanguagePost(Category post, Int32 languageId)
    {
        ...
        string sql = "UPDATE dbo.categories_detail SET title = @title, main_content = @main_content, meta_description = @meta_description, "
            + "meta_keywords = @meta_keywords, page_name = @page_name, date_added = @date_added, page_views = @page_views, inactive = @inactive "
    ...
    public static void UpdatePageviews(Int32 id, Int32 languageId, Int32 pageViews)
    {
        // Create the connection and the sql statement
        string connection = Tools.GetConnectionString();
        string sql = "UPDATE dbo.categories_detail SET page_views = @page_views "
            + "WHERE category_id = @category_id AND language_id = @language_id;";
    ...
    public static string GetValidSortField(string sortField)
    {
        ...
        if (sortField != "id" && sortField != "date_added" && sortField != "page_views" && sortField != "title" && sortField != "inactive")
        {
            sortField = "id";
        }
